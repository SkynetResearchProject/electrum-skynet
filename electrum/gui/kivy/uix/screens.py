...
class SendScreen(CScreen, Logger):
    ...
    def read_invoice(self):
        ...
        if not address:
            self.app.show_error(_('Recipient not specified.') + ' ' + _('Please scan a Skynet-coin address or a payment request'))
            return
        if not self.amount:
            ...
            if self.is_lightning:
                ...
                if self.payment_request:
                    ...
                    if not bitcoin.is_address(address):
                        self.app.show_error(_('Invalid Skynet-coin Address') + ':\n' + address)
                        return
